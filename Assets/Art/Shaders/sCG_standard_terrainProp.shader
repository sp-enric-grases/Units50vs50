...
Shader "socialPointCG/sCG_standard_terrainProp"
{
	Properties
	{
		...
		_SnowHeight("GroundMap level", float) = 0.75
		_SnowBlending("GroundMap blending", float) = 2
		_SnowContrast("GroundMap contrast", float) = 1
	...
	SubShader
	{
		...
		*/
		...
		void vert(inout appdata_full v, out Input o)
		{
			...
			float NdotT = dot(normalize(mul(unity_ObjectToWorld, float4(v.normal,0.0))), float4(0,1,0,0));

			o.NdotTop = saturate(smoothstep(_SnowHeight - _SnowBlending * 0.5, _SnowHeight + _SnowBlending * 0.5, NdotT));
		}
		...
		*/
		...
		void surf(Input IN, inout SurfaceOutputStandard o) 
		{
			...
			//float snowMask = pow(saturate(IN.NdotTop + CM.r * IN.NdotTop), _SnowContrast);
			float snowMask = saturate(IN.NdotTop + CM.r * IN.NdotTop);

			snowMask = pow(snowMask * CM.b, _SnowContrast);

			//DF = lerp(DF, DF_grass, CM.g); 

			DF = lerp(DF, DF_grass, snowMask);
			...
			fixed detail = lerp(detail_rock, detail_grass, snowMask);
